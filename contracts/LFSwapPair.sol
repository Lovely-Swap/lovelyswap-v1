...
contract LFSwapPair is ILFPair, LFSwapERC20 {
	...
	function mint(address to) external onlyActive(to) lock returns (uint256 liquidity) {
	...
	// this low-level function should be called from a contract which performs important safety checks
	function burn(address to) external onlyActive(to) lock returns (uint256 amount0, uint256 amount1) {
		(uint112 _reserve0, uint112 _reserve1, ) = getReserves(); // gas savings
	...
	function skim(address to) external onlyActive(to) lock {
		address _token0 = token0; // gas savings
		address _token1 = token1; // gas savings
	...
	function sync() external onlyActive(msg.sender) lock {
